...
from logical_types import Condition, State, Card, prefix_pool, suffix_pool, any_state, only_color
...
def add_probs(s1: State, pool: List[tuple[List | bool, List | bool]]) -> float:
    ...
    per_b = 1 / len(pool)
    for pp, sp in pool:
        p0 = state_chance(s1.prefixes, _pp=pp, _sp=sp) if type(
            pp) is list else 1
        s0 = state_chance(s1.suffixes, _pp=pp, _sp=sp) if type(
            sp) is list else 1
